...
def initdb():
    ...
    if not(config.get('Server Config','server')) or not(config.get('Server Config','user')) or not(config.get('Server Config','password')) or not(config.get('Server Config','db')):
        _sys.stderr.write('Configuration file error. Please check the configuration file.\n')
        _sys.exit(-1)
    else:
...
def GreatCircDist(loc1,loc2):
    ...
    ha=_math.sin(rdlat/2) * _math.sin(rdlat/2) + _math.cos(_math.radians(loc1[1])) * _math.cos(_math.radians(loc1[1]))*_math.sin(rdlong/2) * _math.sin(rdlong/2)
